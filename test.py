...
from huxel.data import get_raw_data, get_batches, split_trainig_test
from huxel.beta_functions import _f_beta
from huxel.huckel import linear_model_pred, _construct_huckel_matrix
...
def _test(n_tr=50, l=0, lr=2e-3, batch_size=2):
    ...
    def f(batch):
        l = 0.0
        for m in batch:
            l = l + m["homo_lumo_grap_ref"]
        return l

    print(f(ms))


def _test_old(n_tr=50, batch_size=100, lr=2e-3, l=0):
